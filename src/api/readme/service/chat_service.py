...
class ChatService:
    ...
    async def _handle_generation(self, messages: list, meta: dict, thread_id: str):
        ...
        try:
            ...
            # 关键修复：使用线程池在线程中处理每个delta，避免阻塞事件循环
            import asyncio
            import concurrent.futures

            # 将同步生成器转换为列表，然后异步处理每个元素
            def get_next_delta():
                """获取下一个delta，如果没有则返回None"""
                try:
    ...
    async def process_chat_stream(self, query: str, meta: dict = None, history: List[dict] = None, thread_id: str = None) -> AsyncGenerator[bytes, None]:
        ...
        if meta and self.need_retrieve(meta):
            ...
            # 在generating阶段返回检索结果
            logger.debug(f"发送召回文档信息，数量: {len(retrieved_docs)}")
            if retrieved_docs:
                logger.debug(f"召回文档详情: {retrieved_docs}")
