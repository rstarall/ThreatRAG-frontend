...
const Chat: React.FC<{ siderWidth: number }> = ({ siderWidth = 300 }) => {
  ...
  const getModelMenuItems = () => {
    // 兜底：即使后端 /chat/models 没返回，也至少展示常用模型
    ...
    modelProviders.forEach(provider => {
      ...
      const fallbackModels = fallbackModelsByProvider[provider] || [];
      const mergedModels = Array.from(new Set([...(providerModels || []), ...fallbackModels]));
      ...
      if (mergedModels.length > 0) {
        items.push({
          ...
          children: mergedModels.map(model => ({
            key: `${provider}-${model}`,
            label: model,
            onClick: () => {
        ...
        >
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            >
              ...
              >
                <span className="max-w-[220px] truncate">{getCurrentModelDisplay()}</span>
                <CaretDownOutlined className="text-xs opacity-60" />
              </Button>
            </Dropdown>
            {/* 知识库选择 */}
